...
export default function SampleLabelingScreen() {
  ...
  const printMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (reprintCount > 0) {
        addLog(
          `Reprint Dispatched: Job [${data.print_job_id}] pushed state=SENT (Cycle count: ${reprintCount})`,
        );
      } else {
              ...
              <div className="border border-slate-200 bg-white rounded-xl shadow-lg divide-y divide-slate-100 overflow-hidden max-h-48 overflow-y-auto relative z-10 animate-fadeIn">
                {searchResults.map((spec) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] bg-emerald-50 text-emerald-700 font-bold px-2 py-0.5 rounded border border-emerald-200/40">
                      RECEIVED
                    </span>
                  </div>
                ))}
                ...
                <div className="bg-amber-50/60 border border-amber-200 rounded-xl p-4 flex items-start gap-3 text-amber-800 animate-fadeIn">
                  ...
                  <div>
                    <h4 className="text-xs font-black">
                      No Elements Found Awaiting Label Assignment
                    </h4>
                    <p className="text-[10px] text-amber-700/90 mt-0.5 leading-relaxed">
                      Verify if the collection vial parameters have already progressed through
                      checkout or ensure the physical verification entry step was fully saved in
                      Step 6.
                    </p>
                  </div>
                </div>
              ...
              <div className="p-8 text-center space-y-4 animate-fadeIn">
                ...
                <div>
                  <h4 className="text-sm font-black text-slate-900">
                    Specimen Labeling Steps Confirmed
                  </h4>
                  <p className="text-xs text-slate-400 mt-1">
                    Status code moved to Labeled / Ready for Queue. Audit trace string LABEL_AFFIXED
                    logged.
                  </p>
                </div>
                ...
                {!hasPrinted && selectedSpecimen && (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <label
                        className="text-xs font-bold text-slate-900 block select-none cursor-pointer"
                        htmlFor="offlineOverride"
                      >
                        Printer Hardware Offline Fallback Loop
                      </label>
                      <p className="text-[10px] text-slate-400 mt-0.5">
                        Check this box to authorize immediate manual labeling verification if
                        hardware link falls offline.
                      </p>
                    </div>
                    <input
                      type="checkbox"
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex justify-center pt-1">
                <span
                  className={`inline-flex items-center gap-1.5 px-3 py-1 border rounded-full font-bold text-[10px] uppercase tracking-wider shadow-2xs ${hasPrinted ? 'bg-emerald-50 text-emerald-700 border-emerald-200/50' : 'bg-slate-50 text-slate-400 border-slate-200/60'}`}
                >
