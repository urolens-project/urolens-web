import { ArrowLeft, Download, Info, ShieldAlert } from 'lucide-react';
...
export function PatientResultDetail({ result, onDownloadPdf }: PatientResultDetailProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Section 2: Particle Count Findings */}
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <h2 className="text-base font-bold text-slate-800 mb-4">
          Urinalysis Particle Count Findings
        </h2>
        ...
        <CellCountTable particleCounts={result.particle_counts} />
      </div>

      {/* Section 3: Laboratory Notes */}
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <h2 className="text-base font-bold text-slate-800 mb-4">
          Laboratory Notes
        </h2>
        {result.confirmation_notes ? (
          ...
          <p className="text-sm text-slate-400 italic">
            No laboratory notes recorded.
          </p>
        )}
      </div>

      {/* Section 4: Analyzed By */}
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <h2 className="text-base font-bold text-slate-800 mb-4">
          Analyzed By
        </h2>
        <div className="inline-block rounded-lg border border-slate-100 bg-slate-50 p-5 text-center min-w-48">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">
            Examined by
          </p>
          <div className="border-b border-slate-300 pb-2 mb-2">
            <span className="text-sm font-semibold text-slate-700">
              {result.analyzed_by || 'Pending'}
            </span>
          </div>
      ...
      {/* Section 5: Smart Diagnosis Unavailable Notice */}
      {result.smart_diagnosis_unavailable && (
        <div className="flex items-start gap-2 rounded-lg bg-slate-50 border border-slate-200 px-4 py-3 text-sm text-slate-600">
          ...
          <span>
            Automated smart diagnosis was not available for this result. Manual
            review by the Medical Technologist has been completed.
          </span>
        </div>
      )}
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 px-5 py-4">
        <p className="text-xs text-slate-500 leading-relaxed">
          UroLens is a clinical decision-support tool. AI-generated particle
          count findings are not substitutes for professional medical judgment.
          All results must be reviewed and confirmed by a licensed Medical
          Technologist and Laboratory Supervisor.
        </p>
      </div>
    </div>
  );
