...
export default function LabRequestForm() {
  ...
  const requestMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      addLog(`Success: Created Lab Request UID [${data.request_uid || 'OK'}]`);
    },
    onError: (error: Error) => {
          ...
          <div className="relative space-y-8">
            ...
            <div className="rounded-2xl bg-slate-50 border border-slate-100 p-5 space-y-4 text-xs">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="bg-amber-50 text-amber-700 border border-amber-200 font-bold px-2 py-0.5 rounded uppercase tracking-wider text-[9px]">
                  PENDING SAMPLE
                </span>
              </div>
            </div>
