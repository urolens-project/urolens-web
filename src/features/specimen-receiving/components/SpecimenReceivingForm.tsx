            ...
            <div className="flex items-start gap-3 text-amber-800">
              ...
              <div>
                ...
                <p className="text-[10px] text-amber-700/90 mt-0.5 leading-relaxed">
                  As dictated by laboratory workflow safeguards, a specimen cannot be logged unless
                  its order has been pre-encoded.
                </p>
              </div>
            </div>
            <button
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            >
              ...
              />
              <span className="text-[10px] font-semibold">Adequate Volume (≥30mL)</span>
            </label>
            ...
            >
              <input
                ...
                className="rounded text-emerald-600 focus:ring-emerald-500 h-3.5 w-3.5"
              />
              <span className="text-[10px] font-semibold">Sterile Container Lid Lock</span>
            </label>
            ...
            >
              <input
                ...
                className="rounded text-emerald-600 focus:ring-emerald-500 h-3.5 w-3.5"
              />
              <span className="text-[10px] font-semibold">Legible Match Profiling</span>
            </label>
          </div>
        </div>
        ...
        <div className="bg-slate-50 p-4 border border-slate-200 rounded-xl flex items-center justify-between">
          <div>
            <label className="text-xs font-bold text-slate-900 block">
              Overall Visual Check Passed Confirmation
            </label>
            <p className="text-[10px] text-slate-400 mt-0.5">
              Toggle to FAILED manually if any specimen parameter shows discrepancy.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="flex items-start gap-2.5 text-red-700">
              ...
              <div>
                ...
                <p className="text-[10px] text-red-500 font-medium mt-0.5">
                  Under code tracking rules, an explicitly chosen failure path is mandatory.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <option value="INSUFFICIENT_VOLUME">
                  INSUFFICIENT_VOLUME (Specimen volume measures below 30mL)
                </option>
                <option value="WRONG_CONTAINER">
                  WRONG_CONTAINER (Non-sterile or unapproved container type used)
                </option>
                <option value="UNLABELED">
                  UNLABELED (Container missing complete patient identification elements)
                </option>
                <option value="OTHER">
                  OTHER (Obvious contamination, leakage, or external damage anomalies)
                </option>
              </select>
      ...
      <div className="bg-slate-50/60 border-t border-slate-100 p-4 flex items-center justify-end gap-3">
        ...
        >
          {isPending
            ? 'Processing Matrix Commit...'
            : visualCheckPassed
              ? 'Confirm Specimen Receipt'
              : 'Execute System Rejection Logs'}
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              The pipeline ledger records have been safely updated inside the cloud.
            </p>
          </div>
        </div>
        ...
        <div className="border-y border-dashed border-slate-200 py-5 space-y-3.5">
          <div className="flex justify-between items-center text-xs">
            ...
            <span
              className={`font-mono font-black px-3 py-1 rounded-lg border tracking-widest text-sm ${isRejected ? 'bg-red-50 text-red-700 border-red-200/50' : 'bg-emerald-50 text-emerald-700 border-emerald-200/50'}`}
            >
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span
              className={`font-extrabold text-[10px] uppercase tracking-wider text-white px-2.5 py-0.5 rounded-md shadow-xs ${isRejected ? 'bg-red-500' : 'bg-emerald-500'}`}
            >
...
export default function SpecimenReceivingForm() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!selectedRequest)
      errors.request =
        'A target lab request authorization record is mandatory before receiving can execute.';
    if (!visualCheckPassed && !rejectionReason)
      errors.rejection = 'Please map a standard structured reason taxonomy code mapping option.';
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex justify-center pt-1">
                <span
                  className={`inline-flex items-center gap-1.5 px-3 py-1 border rounded-full font-bold text-[10px] uppercase tracking-wider shadow-2xs ${visualCheckPassed ? 'bg-emerald-50 text-emerald-700 border-emerald-200/50' : 'bg-red-50 text-red-700 border-red-200/50'}`}
                >
                  <Clipboard className="h-3 w-3" />{' '}
                  {visualCheckPassed ? 'Inspection Passed' : 'Flagged Rejection'}
                </span>
              </div>
            </div>
          </div>
